...
public class RecipeCompilerTest {
  ...
  @Test
  public void testAggregateStatsWithOptionsCompilation() throws Exception {
    String[] recipe = {
        "aggregate-stats :data_size :response_time :total_size :total_time size-unit 'GB' time-unit 's' average true"
    };
    CompileStatus status = TestingRig.compile(recipe);
    assertTrue(status.isSuccess());
  }
